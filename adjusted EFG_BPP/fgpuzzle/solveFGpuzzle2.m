...
for i=1:iterNum
    ...
    fgMap_hypotheses(:,:,1) = fgMap_hypotheses(:,:,1) * 0.0; % set MSER to be zero
    ...
    fgMap_i=fgMap_hypotheses(:,:,1)+2*fgMap_hypotheses(:,:,2)+0.5*(fgMap_hypotheses(:,:,3)+fgMap_hypotheses(:,:,4));    %note that fgMap_i is negative map for reverseFG==1 !!!   
    ...
    %fgMask=fgMap_hypotheses(:,:,1);     %scheme 1: use <merged> hypothesis for background propagation.
    fgMask=fgMap_i;
